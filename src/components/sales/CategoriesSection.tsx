...
const categories = [
  { name: "Papelaria Fofa", icon: PenTool },
  { name: "Garrafas e Marmitas", icon: Coffee },
  { name: "Acessórios e Laços", icon: Ribbon },
  { name: "Bijouterias e Semijoias", icon: Gem },
  { name: "Mochilas e Malas", icon: Briefcase },
  { name: "Películas e Capinhas", icon: Smartphone },
  { name: "Presentes e Pelúcias", icon: Gift },
  { name: "Embalagens Personalizadas", icon: Package },
  { name: "Maquiagem", icon: Palette },
  { name: "Unhas e Cílios", icon: Sparkles },
  { name: "Cosméticos e Perfumes", icon: Heart },
  { name: "Perucas e Cabelos", icon: Scissors },
  { name: "Eletrônicos", icon: Cpu },
  { name: "Games e Acessórios", icon: Gamepad2 },
        ...
        <div className="grid grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4 md:gap-6">
          {categories.map((category, index) => (
            <a
              key={index}
              href="#pricing"
              className="group flex flex-col items-center gap-3 p-4 md:p-6 glass-card rounded-2xl neon-border hover-glow transition-all duration-300 hover:-translate-y-1 cursor-pointer relative overflow-hidden animate-fade-in"
            ...
            >
              ...
              <div 
                className="relative w-16 h-16 md:w-20 md:h-20 rounded-full bg-gradient-to-br from-neon-cyan/20 to-neon-blue/20 border border-neon-cyan/30 flex items-center justify-center group-hover:scale-110 transition-transform duration-300 group-hover:border-neon-cyan/60"
                style={{
